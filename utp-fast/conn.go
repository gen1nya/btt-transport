...
func (c *Conn) wndSize() uint32 {
	// Always advertise max window. Our application (VPN proxy) reads
	// immediately, so receiver-side flow control only causes unnecessary
	// throughput oscillation (sawtooth pattern).
	return 0xFFFFFFFF
}

func (c *Conn) makePacket(_type st, connID, seqNr uint16, payload []byte) (p []byte) {
	var selAck selectiveAckBitmask
	// Only scan up to maxSACKBytes*8 bits to keep headers small.
	sackLimit := len(c.inbound)
	if sackLimit > maxSACKBytes*8 {
		sackLimit = maxSACKBytes * 8
	}
	for i := 1; i < sackLimit; i++ {
		if c.inbound[i].seen {
			selAck.SetBit(i - 1)
		}
	}
	...
	if len(selAck.Bytes) != 0 {
		...
		// Truncate to maxSACKBytes to keep payload efficient.
		if len(selAck.Bytes) > maxSACKBytes {
			selAck.Bytes = selAck.Bytes[:maxSACKBytes]
		}
...
func (c *Conn) processInbound() {
	// Consume consecutive next packets. No readBuf size limit — our
	// application reads promptly, and back-pressure here causes the
	// sender to stall (sawtooth throughput).
	for !c.gotFin.IsSet() && len(c.inbound) > 0 && c.inbound[0].seen {
		c.ack_nr++
		p := c.inbound[0]
