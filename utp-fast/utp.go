...
const (
	...
	// ACK. Cap SACK bitmask at 32 bytes (256 bits) regardless of
	// maxUnackedInbound to keep payload size efficient.
	maxSACKBytes   = 32
	maxHeaderSize  = 20 + 2 + maxSACKBytes
	maxPayloadSize = minMTU - maxHeaderSize
	maxRecvSize    = 0x2000

	// Maximum out-of-order packets to buffer.
	// Increased from 256 for higher throughput on long-RTT links.
	// BDP at 100 Mbps / 150ms RTT ≈ 1.8 MB ≈ 1300 packets.
	maxUnackedInbound = 1024
	maxUnackedSends   = 1024

	readBufferLen = 16 << 20 // ~16MiB
